...
3. Notifications 도메인: 교통/점심/도서관 CRUD (`/me/notifications/*`) + JSONB config + history 테이블 + **F-17 `/me/notifications/history` Plan A 임시 구현** (§B-3)
...
### B-3. F-17 알림 발송 이력 조회 — Plan A 임시 구현 완료, 정식 컬럼화 후속 PR 대기
- **완료**: `GET /api/v1/me/notifications/history` 신규 (`backend/app/domains/notifications/router.py:150`). Query 파라미터 `date_from`/`date_to`/`type`/`status`/`limit` (1~100 cap). 응답 `list[NotificationHistoryRead]` (id·type·sent_at·status·failure_reason·payload·notification_id·immediate_send_request_id). 단순 list, cursor 페이지네이션 미도입.
- **type 도출**: `notification_history` 본체에 `type` 컬럼이 없어 `Notification` / `ImmediateSendRequest` 와 LEFT JOIN 후 `func.coalesce` 로 도출(`repository.py:list_history_for_user`). 양 FK 가 모두 NULL 인 관리자(F-22) row 는 응답에서 제외 (`WHERE notification_id IS NOT NULL OR immediate_send_request_id IS NOT NULL`).
- **30일 상한 정책**: 둘 다 미지정 시 `now-30d~now`, 한쪽만 지정 시 다른 쪽 ±30d 보정. `date_from > date_to` 또는 범위 >30d 면 `InvalidHistoryDateRange`(422 `INVALID_HISTORY_DATE_RANGE`). `date_to` 는 inclusive day → 다음 자정 직전(`< sent_at_to`) 비교.
- **payload**: raw JSONB 그대로 노출. 프론트가 type 별로 파싱한다. 적재 형식 합의 전이라 의도적으로 요약 추출 안 함.
- **회귀 가드**: `test_history_router.py` 11건 + `test_history_service.py` 6건 (time-machine 으로 윈도우 결정적 검증). 타 유저 격리·관리자 row 제외·JOIN 으로 immediate type 도출 모두 커버.
- **후속 작업 (§E-1 합의 후)**:
  - `notification_history.type` 컬럼 비정규화 + alembic 마이그레이션 + 봇 INSERT 측 type 채우기 (LEFT JOIN 제거). bot roadmap §C-1 과 묶임.
  - payload JSONB 정식 키 합의 + 응답에 `content_summary` 추출 필드 추가 (백엔드 단독 가능, 봇과 무관).
  - cursor 페이지네이션 (`?cursor=base64(last_id)&limit=`) — 30일 안에 100건 초과가 흔치 않다면 보류.
  - rate limit (동일 user_id 의 history 조회 폭주 방지) — 정책 결정이 먼저.
...
**~~C-1 (Redis)~~ → ~~C-2 (로그아웃/refresh)~~ → ~~OAuth state Redis 단발성 전환~~ → ~~B-1 (require_role)~~ → ~~D-1 (get_current_user 리팩터)~~ → ~~B-3 (F-17 history 라우터 Plan A)~~ → A-2 (jwt 길이) → B-2 (활성 시간대, 봇 합의 후) → D-2 (alembic 왕복) → D-3 (CI) → A-1 (PATCH /me) → E-* (봇 합의 후) → B-3 후속(§E-1 합의 후 type 컬럼 비정규화 + payload 요약 추출)**
