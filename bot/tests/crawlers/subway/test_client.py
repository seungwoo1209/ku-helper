...
@pytest.mark.asyncio
async def test_fetch_arrivals_info_000_returns_subway_arrivals(
...
@pytest.mark.asyncio
async def test_fetch_arrivals_info_100_raises_auth_failed(
    redis: fakeredis.aioredis.FakeRedis,
) -> None:
...
@pytest.mark.asyncio
async def test_fetch_arrivals_http_5xx_raises_unavailable(
    redis: fakeredis.aioredis.FakeRedis,
) -> None:
...
def test_subway_client_raises_auth_failed_when_key_is_none(
    redis: fakeredis.aioredis.FakeRedis,
) -> None:
    """subway_api_key = None → SubwayClient 생성 시 SubwayApiAuthFailed."""
    with pytest.raises(SubwayApiAuthFailed):
        SubwayClient(httpx.AsyncClient(), _make_settings(api_key=None), redis)


def test_subway_client_raises_auth_failed_when_key_is_empty(
    redis: fakeredis.aioredis.FakeRedis,
) -> None:
    """subway_api_key = '' → SubwayClient 생성 시 SubwayApiAuthFailed."""
    with pytest.raises(SubwayApiAuthFailed):
        SubwayClient(httpx.AsyncClient(), _make_settings(api_key=""), redis)
...
@pytest.mark.asyncio
async def test_fetch_arrivals_unknown_subway_id_uses_unknown_label(
    redis: fakeredis.aioredis.FakeRedis,
) -> None:
...
@pytest.mark.asyncio
async def test_row_to_arrival_parses_train_line_name_and_received_at(
    redis: fakeredis.aioredis.FakeRedis,
) -> None:
...
@pytest.mark.asyncio
async def test_row_to_arrival_empty_recptn_dt_gives_none_received_at(
    redis: fakeredis.aioredis.FakeRedis,
) -> None:
