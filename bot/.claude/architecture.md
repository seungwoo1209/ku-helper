...
- Crawler가 실패하면 Redis 카운터 `crawler_fail:{source}` INCR + TTL 5분.
- 3회 누적 감지 시 관리자 DM 작업을 Sender 큐에 적재(일반 사용자 알림과 같은 큐).
- 동일 장애 중복 알림 방지: Redis 키 `crawler_alert_cooldown:{source}` TTL 30분.
