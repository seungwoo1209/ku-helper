...
- terraform 은 시크릿 파라미터를 placeholder(`PLACEHOLDER_SET_VIA_CLI`)로만 생성하고 `lifecycle { ignore_changes = [value] }` 로 이후 값 변경을 무시한다. 정의 위치는 `infra/persistent/ssm_secrets.tf` 다.
- 시크릿 파라미터를 ephemeral 이 아니라 **persistent 모듈**에 둔 이유는, persistent 가 spin-up/teardown 사이클로 삭제되지 않아 1회 등록한 값이 계속 유지되기 때문이다. teardown 후 다시 spin-up 해도 재등록이 필요 없다.
...
#### 시크릿 최초 등록 (persistent apply 직후 1회)

`infra/persistent` apply 로 placeholder 가 생성된 뒤, 아래 명령으로 실제 값을 등록한다. 필수 시크릿은 반드시 등록하고, 선택 시크릿은 해당 알림 기능을 쓸 때만 등록한다(미등록 시 placeholder 가 남으며 애플리케이션이 미설정으로 처리).
