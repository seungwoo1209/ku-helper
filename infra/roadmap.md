...
spin-up.yml 실행 전 GitHub Actions Secrets 에 추가로 등록해야 할 항목 (워크플로 가 -var 로 terraform 에 주입, terraform 이 SSM Parameter Store 로 동기화):
| Secret | 용도 |
| --- | --- |
| `KU_HELPER_JWT_SECRET` | backend JWT 서명. 32바이트 이상 권장. |
| `KU_HELPER_DISCORD_CLIENT_ID` | Discord OAuth Client ID (값 비밀 아님이지만 일관성 위해 secret). |
| `KU_HELPER_DISCORD_CLIENT_SECRET` | Discord OAuth Client Secret. |
| `KU_HELPER_DISCORD_BOT_TOKEN` | Bot 토큰. |
| `KU_HELPER_SUBWAY_API_KEY` | 서울 공공 API 키 (선택). |
| `KU_HELPER_NAVER_SEARCH_CLIENT_ID` | Naver 지역검색 (선택). |
| `KU_HELPER_NAVER_SEARCH_CLIENT_SECRET` | Naver 지역검색 (선택). |
| `KU_HELPER_LIBRARY_SEAT_URL` | 도서관 좌석 API URL (선택). |
| `KU_HELPER_ADMIN_DISCORD_IDS` | 관리자 Discord ID(콤마 구분, 선택). |
| `KU_HELPER_GHCR_PAT` | GHCR `read:packages` PAT — EC2 가 docker login 시 사용. |
